...
from src.paths import TEST_DATA  # noqa: E402

STATE: Dict[str, Any] = {"agent": None, "error": None}
...
async def lifespan(app: FastAPI):
    ...
    try:
        STATE["agent"] = RecoveryAgent(adapter=get_adapter())
...
def simulate_batch(request: BatchRequest):
    """Run the agent over a batch of failed transactions."""
...
def simulate_seed(n: int = Query(default=200, ge=1, le=2000)):
    ...
    decisions = _agent().decide_batch(df, log=True)   # feeds GET /decisions
    return {
...
def breakdown(n: int = Query(default=500, ge=1, le=2000)):
    ...
    decisions = _agent().decide_batch(pd.read_csv(TEST_DATA).head(n))
    return {
        "n": int(len(decisions)),
